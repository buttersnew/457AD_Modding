...
#convert slowly to ideal prosperity
(0.25, [#95hours on average
      ...
      (try_begin),
        (try_begin),
          ...
          (try_begin),
            ...
            (try_end),
          (else_try),
            (party_slot_eq, "$g_center_trigger_taxes", slot_party_type, spt_castle),
            (assign, ":cur_rents", 1200),
          (else_try),
            (party_slot_eq, "$g_center_trigger_taxes", slot_party_type, spt_town),
            (assign, ":cur_rents", 2400),
          (try_end),

          (party_get_slot, ":prosperity", "$g_center_trigger_taxes", slot_town_prosperity), #prosperty changes between 0..100
          ...
          (val_div, ":cur_rents", 120),#Prosperity of 100 gives the default values
            ...
            (try_begin), #decreases taxes by 20% if center is different religion than the player
              (party_slot_eq, "$g_center_trigger_taxes", slot_town_lord, "trp_player"),
              (party_get_slot, ":center_religion", "$g_center_trigger_taxes", slot_center_religion),
              (troop_get_slot, ":player_religion", "trp_player", slot_troop_religion),
              (try_begin),
                (eq, ":center_religion", ":player_religion"), #shares a religion - aka same
              (else_try),
                (neq, ":center_religion", ":player_religion"), #does not share a religion
                (val_mul, ":cur_rents", 3),
                (val_div, ":cur_rents", 4),                
              (try_end),
            (try_end),
          ...
          (try_begin),
            ...
            (party_get_slot, ":tax_rate", "$g_center_trigger_taxes", dplmc_slot_center_taxation),
            ...
            (call_script, "script_change_center_prosperity", "$g_center_trigger_taxes", ":tax_rate"),
            ...
            (try_begin),
              ...
              (try_begin),
                ...
                (else_try),
                  ...
                  (party_add_members, "$g_center_trigger_taxes", "trp_townsman", ":random"),
                  (store_random_in_range, ":random",0, ":player_level"),
              ...
              (end_try),
            (try_end),
            (call_script, "script_change_player_relation_with_center", "$g_center_trigger_taxes", ":tax_rate"),
          (try_end),

          (try_begin), #no taxes for infested villages and towns
            (party_slot_ge, "$g_center_trigger_taxes", slot_village_infested_by_bandits, 1),
            (assign,":accumulated_rents", 0),
          (try_end),
          ##diplomacy end
          (party_set_slot, "$g_center_trigger_taxes", slot_center_accumulated_rents, ":accumulated_rents"),
        (try_end),
...
#NPC companion changes end
 ...
 #Update how good a target player is for bandits
  ...
  (0.03,
   [#12/380
    (try_begin),
        (neg|is_between, "$g_center_upgrades", centers_begin, centers_end),
        (assign, "$g_center_upgrades", centers_begin),
    (try_end),   
    
    # (try_for_range, ":center_no", centers_begin, centers_end),
    (try_begin),
      (party_get_slot, ":cur_improvement", "$g_center_upgrades", slot_center_current_improvement),
      (gt, ":cur_improvement", 0),
      (party_get_slot, ":cur_improvement_end_time", "$g_center_upgrades", slot_center_improvement_end_hour),
      (store_current_hours, ":cur_hours"),
      (ge, ":cur_hours", ":cur_improvement_end_time"),
      (party_set_slot, "$g_center_upgrades", ":cur_improvement", 1),
      (party_set_slot, "$g_center_upgrades", slot_center_current_improvement, 0),
      (call_script, "script_get_improvement_details", ":cur_improvement"),
      (try_begin),
        (party_slot_eq, "$g_center_upgrades", slot_town_lord, "trp_player"),
        (str_store_party_name, s4, "$g_center_upgrades"),
        (display_log_message, "@Building of {s0} in {s4} has been completed."),
      (try_end),

        (try_begin),
            (eq, ":cur_improvement", slot_center_has_temple1),
            (party_set_slot, "$g_center_upgrades", slot_center_religion, slot_religion_chalcedonian),
            (str_store_party_name, s4, "$g_center_upgrades"),
            (display_log_message, "@{s4} converted to Chalcedonian Christianity."),
        (else_try),
            (eq, ":cur_improvement", slot_center_has_temple2),
            (party_set_slot, "$g_center_upgrades", slot_center_religion, slot_religion_paganism),
            (display_log_message, "@{s4} converted to Paganism."),
        (else_try),
            (eq, ":cur_improvement", slot_center_has_temple3),
            (party_set_slot, "$g_center_upgrades", slot_center_religion, slot_religion_arianism), 
            (display_log_message, "@{s4} converted to Arianism."),
        (else_try),
            (eq, ":cur_improvement", slot_center_has_temple4),
            (party_set_slot, "$g_center_upgrades", slot_center_religion, slot_religion_zoroastrianism),  
            (display_log_message, "@{s4} converted to Zoroastrianism."),
        (else_try),
            (eq, ":cur_improvement", slot_center_has_temple5),
            (party_set_slot, "$g_center_upgrades", slot_center_religion, slot_religion_coptic),  
            (display_log_message, "@{s4} converted to Coptic Christianity."),
        (else_try),
            (eq, ":cur_improvement", slot_center_has_temple6),
            (party_set_slot, "$g_center_upgrades", slot_center_religion, slot_religion_roman_paganism),   
            (display_log_message, "@{s4} converted to Roman Paganism."),
        (try_end), 
     
      (try_begin),
        (is_between, "$g_center_upgrades", villages_begin, villages_end),
        (try_begin),
          (eq, ":cur_improvement", slot_center_has_fish_pond),
          (call_script, "script_change_center_prosperity", "$g_center_upgrades", 5),
        (else_try), #SB : show garrison for debug
          (eq, ":cur_improvement", slot_center_has_manor),
          (party_set_flags, "$g_center_upgrades", pf_hide_defenders, 0),
        # (else_try),
    ...
    #pig packing
    (party_set_slot, "$g_center_upgrades", dplmc_slot_village_reserved_by_recruiter, 0),
    (val_add, "$g_center_upgrades", 1),
    ]),
...
(0.07,
  ...
  #24/380, daily on average
    ...
    (this_or_next|party_slot_eq, ":center_no", slot_center_has_temple4, 1),
    (this_or_next|party_slot_eq, ":center_no", slot_center_has_temple5, 1),
    (party_slot_eq, ":center_no", slot_center_has_temple6, 1),
    (store_random_in_range, ":prosperity_boost", 0, 3),
    (call_script, "script_change_center_prosperity", ":center_no", ":prosperity_boost"),
  ]),

  (0.25, #checks every (4) days - making sure that relations do not decrease at an insane rate
    [
