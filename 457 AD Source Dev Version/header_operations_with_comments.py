...
else_try                =    5  # (else_try),
                                # If conditional operations in the conditional block fail, this block of code will be executed. Works also within try_for_agents and try_for_range.
...
try_for_agents          =   12  # (try_for_agents, <destination>, [<position_no>], [<radius_fixed_point>]), #avoid using pos0
                                # Runs a cycle, iterating all active agents on the scene. Iteration works for active agents alive and dead. Don't use check agent_is_active. It will always return true.
...
is_cheat_mode_enabled                 =   53  # (is_cheat_mode_enabled),
                                              # Checks if Cheat Mode is currently enabled in the game.
...
position_get_distance_to_ground_level       =  793  # (position_get_distance_to_ground_level, <destination>, <position>),
                                                    # This will measure the distance between position and the ground level, taking scene props and their collision meshes into account. Retrieves a negative value if underground. Operation only works on the scenes and cannot be used on the global map.
...
scene_prop_fade_out                         = 1822  # (scene_prop_fade_out, <scene_prop_id>, <fade_out_time>),
                                                    # Version 1.153+. Makes the scene prop instance disappear within specified time. The specified prop's main meta mesh (base mesh) fades, leaving the submeshes still visible.
...
agent_get_look_position                  = 1709  # (agent_get_look_position, <position>, <agent_id>),
                                                 # Retrieves the position that the agent is currently looking at. It is a position that is located centrally horizontally and vertically on the bottom of the model. Basically, it is a position rotated with the agents direction but located underneath the feet.
